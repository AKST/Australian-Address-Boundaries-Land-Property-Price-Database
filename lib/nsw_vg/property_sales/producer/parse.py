...
class PropertySalesParser:
    ...
    async def get_data_from_file(self: Self):
        ...
        try:
            ...
            columns, syntax = get_columns_and_syntax(date, year)
            if columns is None or syntax is None:
                return
...
def get_columns_and_syntax(download_date: Optional[datetime], year: int) -> Tuple[FactoryMap, Syntax] | Tuple[None, None]:
    if download_date is None and year <= 2001:
        return ps.t_1990, SYNTAX_1990
    elif download_date is None:
        raise TypeError('missing datetime, cannot parse')

    match (download_date, year):
