...
class DateRangeParamTestCase(unittest.TestCase):
    def test_can_cache(self):
        param = DateRangeParam(YM(2010, 1), YM(2011, 1), get_now)
        self.assertTrue(param.can_cache())

        param = DateRangeParam(YM(2010, 1), YM(2013, 1), get_now)
        self.assertFalse(param.can_cache())

    def test_shard(self):
        m12_apart = DateRangeParam(YM(2010, 5), YM(2011, 5), get_now)
        ...
        y20_apart = DateRangeParam(YM(1950, 1), YM(1970, 1), get_now)
        ...
        y21_apart = DateRangeParam(YM(1950, 1), YM(1971, 1), get_now)
        ...
        y300_apart = DateRangeParam(YM(1000, 1), YM(1300, 1), get_now)
        ...
        y301_apart = DateRangeParam(YM(1000, 1), YM(1301, 1), get_now)
